...
TEST_CASE("Benchmark: Measuring slowdown for insertOrdered and merge",
          "[weight=0][.][bench]") {
...

    {
      ...
      if (studentList0 != list0_correct) {
        std::cout << "WARNING: It appears insertOrdered isn't correctly "
                     "implemented yet.\n"
                  << "  The running times below may not be meaningful."
                  << std::endl;
      }
    }
  ...
  SECTION("Timing merge") {
...

    {
      ...
      if (studentList0 != list0_correct) {
        std::cout
            << "WARNING: It appears merge isn't correctly implemented yet.\n"
...
// This is hidden because of the [.] tag.
// You can run it explicitly with: ./test [bench]
TEST_CASE("Benchmark: Measuring slowdown for sorting algorithms",
          "[weight=0][.][bench]") {
    ...
    // https://en.cppreference.com/w/cpp/chrono/duration/duration_cast
    {
      ...
      for (int i = 0; i < NUM_TEST_RUNS; i++) {
        ...
        if (!sortedList.isSorted())
          std::cout << "WARNING: mergeSortIterative result not sorted."
                    << std::endl;
      }
      auto stop_time = std::chrono::high_resolution_clock::now();
      std::chrono::duration<double, std::milli> dur_ms = stop_time - start_time;
...
TEST_CASE("Testing insertOrdered: Insert at front", "[weight=1]") {
  ...
  SECTION(
      "Checking that the list prev links and tail pointer are being set "
      "correctly") {
...
TEST_CASE("Testing merge: Left list empty; right list non-empty",
          "[weight=1]") {
  LinkedList<int> left;
  LinkedList<int> right;
  right.pushBack(1);
...
TEST_CASE("Testing merge: Left list non-empty; right list empty",
          "[weight=1]") {
  LinkedList<int> left;
  left.pushBack(1);
  left.pushBack(2);
...
TEST_CASE("Testing merge: Left and right lists non-empty; same size",
          "[weight=1]") {
  LinkedList<int> left;
  left.pushBack(1);
  left.pushBack(5);
...
TEST_CASE("Testing merge: Left and right lists non-empty; left list is longer",
          "[weight=1]") {
  LinkedList<int> left;
  left.pushBack(1);
  left.pushBack(5);
...
TEST_CASE("Testing merge: Left and right lists non-empty; right list is longer",
          "[weight=1]") {
  LinkedList<int> left;
  left.pushBack(1);
  left.pushBack(20);
