...
// Based on Zephyr test runner by Prof. Wade Fagen-Ulmschneider and the CS 225
// Course Staff Based on Catch2 unit testing framework
...
// TEST_CASE("Should have ownership permissions to run your main executable",
// "[weight=1]") {
...
TEST_CASE("Should find \"Greetings from Illinois!\" in standard output",
          "[weight=1]") {
  int result = std::system(
      "./main | tr \'\n\' \' \' | tr A-Z a-z | sed \'s/[^a-z]//g\' | grep "
      "greetingsfromillinois > /dev/null");
  REQUIRE(0 == result);
}
