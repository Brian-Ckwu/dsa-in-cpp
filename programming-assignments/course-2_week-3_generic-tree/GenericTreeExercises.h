...
static void treeFactory(GenericTree<int>& tree) {
  tree.clear();
  // root level
  auto rootNodePtr = tree.createRoot(4);
  // second level
  auto eightNodePtr = rootNodePtr->addChild(8);
  rootNodePtr->addChild(15);
  // third level
  auto sixteenNodePtr = eightNodePtr->addChild(16);
  eightNodePtr->addChild(23);
  // last level
  sixteenNodePtr->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> todos;
  todos.push(rootNodePtr);
  while (!todos.empty()) {
    auto nodePtr = todos.front();
    todos.pop();
    results.push_back(nodePtr->data);
    for (auto childNodePtr : nodePtr->childrenPtrs) {
      todos.push(childNodePtr);
    }
  }
