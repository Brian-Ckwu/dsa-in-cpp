...
template <class T>
struct is_double
    : std::integral_constant<bool, std::is_same<double, T>::value> {};
}  // namespace uiuc_testing
...
// They evaluate constructors, but the Week 1-2 content doesn't cover
// constructors currently.
#undef DO_PART1_CTOR_TESTS
#ifdef DO_PART1_CTOR_TESTS

// HSLAPixel default constructor
TEST_CASE("HSLAPixel's default constructor should create a white pixel",
          "[weight=1]") {
  HSLAPixel pixel;
  REQUIRE(pixel.l == 1);
}

TEST_CASE("HSLAPixel's default constructor should create an opaque pixel",
          "[weight=1]") {
  HSLAPixel pixel;
  REQUIRE(pixel.a == 1);
}
...
TEST_CASE(
    "HSLAPixel's HSL constructor should create a pixel with provided HSL "
    "values",
    "[weight=1]") {
...
TEST_CASE("HSLAPixel's HSL constructor should create an opaque pixel",
          "[weight=1]") {
  HSLAPixel pixel(260, 0.8, 0.7);
  REQUIRE(pixel.a == 1);
}
...
TEST_CASE(
    "HSLAPixel's HSLA constructor should create a pixel with provided HSLA "
    "values",
    "[weight=1]") {
