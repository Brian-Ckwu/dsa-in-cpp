...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      // Calculate the luminance level
      double l = (dist > 160.0) ? 0.2 : 1.0 - 0.005 * dist;
...
PNG illinify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if ((blue >= pixel.h) && (pixel.h >= orange)) {
        if ((blue - pixel.h) < (pixel.h - orange)) {
          pixel.h = blue;
        } else {
          pixel.h = orange;
        }
      } else if (orange > pixel.h) {
        pixel.h = orange;
      } else {
        if ((pixel.h - blue) < (orange - pixel.h + 360)) {
          pixel.h = blue;
        } else {
          pixel.h = orange;
        }
